...
frmSplash.Label5.Caption = "Проставляем сальдо на начало всем у кого есть расхождения всем"
frmSplash.Label5.Refresh
...
frmSplash.Label5.Caption = "Проставляем 0 для пустых № кв"
...
frmSplash.Label5.Caption = "Заполняем значения тарифов"
...
'Удаляю строки начислений введенные из документов оплаты
'Mconn.Execute ("DELETE Adding.* FROM Adding INNER JOIN Doc ON Adding.KodDoc = Doc.Key")

'Удаляю строки начислений введенные из табличных документов
...
Mask = "*.amd"
File = Dir$(Folder & Mask, vbReadOnly) 'в т.ч. RO
Do While Len(File)
...
Do While Len(File)
    ...
    File = Dir$
Loop
'MsgBox "Найдено файлов: " & nf & vbLf & "Удалено файлов: " & nd
End Sub
