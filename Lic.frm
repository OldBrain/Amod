...
Begin VB.Form Lic 
   ...
   Picture         =   "Lic.frx":030A
   ScaleHeight     =   8028
   ScaleWidth      =   10752
   Begin VB.CommandButton Command9 
      Caption         =   "Апхив начисления"
      Height          =   612
      Left            =   2280
      TabIndex        =   42
      Top             =   5880
   ...
   Begin VB.Menu Печать 
      Caption         =   "Печать"
      Begin VB.Menu Lift 
         Caption         =   "Извещение Астраханьлифт "
         Index           =   5
      End
      Begin VB.Menu lift_izv 
         Caption         =   "Извещение Астраханьлифт ЗЛ"
         Index           =   4
      End
...
SaldoArh.Open ("SELECT Saldo_Arh.KodKV, Saldo_Arh.KodKat, Saldo_Arh.SK From Saldo_Arh WHERE (((Saldo_Arh.KodKV)=" + Filter.Nm + ") AND ((Saldo_Arh.KodKat)=" + fg1.TextMatrix(FGS, 22) + "))"), Mconn, adOpenKeyset, adLockPessimistic
              If SaldoArh.RecordCount = 0 Then
        

Mconn.Execute ("INSERT INTO Saldo_Arh ( KodKV, KodKat, SK ) SELECT " + Filter.Nm + " AS Выражение1, " + fg1.TextMatrix(FGS, 22) + " AS Выражение2, " + Replace(Text1.Text, ",", ".") + " AS Выражение3")
                          Else
Mconn.Execute ("UPDATE Saldo_Arh SET Saldo_Arh.SK = " + Replace(Text1.Text, ",", ".") + " WHERE (((Saldo_Arh.KodKV)=" + Filter.Nm + ") AND ((Saldo_Arh.KodKat)=" + fg1.TextMatrix(FGS, 22) + "))")
                         End If
End If
...
TableWord.Cell(5, 2).Range.Text = Lic.Label11
...
'Удаляем пустые файлы из DATA\AEHIV
Call MainForm.DelDocs400_2
...
Mconn.Execute ("INSERT INTO arh_rep SELECT Adding.* FROM Adding IN '" + App.Path + "\Data\kvartplata.amd' WHERE (((Adding.KodKv)=" + Filter.Nm + "));")
...
If Lic.fg1.TextMatrix(rw, 15) = "" Then
...
MainForm.II = 0
MainForm.Pi = 0
MainForm.Ostatok = Lic.fg1.TextMatrix(rw, 15)



'проставляем ед. измерения
Mconn.Execute ("UPDATE Adding INNER JOIN Nachisleniy ON Adding.KodN = Nachisleniy.Kod SET Adding.edizm = [Nachisleniy]![edizm] WHERE (((Adding.KodKv)=" + Filter.Nm + "))")



                'Если счетчик
                
' проставляем норматив из тарифа
Mconn.Execute ("UPDATE Adding INNER JOIN Tarif ON (Tarif.KodDOM = Adding.TipDomKod) AND (Adding.TipKvKod = Tarif.KodKV) AND (Adding.KodKat = Tarif.KodKat) SET Adding.norm = [Tarif]![norm] WHERE (((Adding.KodKv)=" + Filter.Nm + "))")
            
        
' Если данных нет то добавляем норматив

'Mconn.Execute ("UPDATE Adding SET Adding.Shc_new = [Adding]![Shc_old]+[Adding]![norm]*[Adding]![Propis] WHERE (((Adding.Sch)='Да') AND (([Adding]![Shc_old]-[Adding]![Shc_new])=0) AND ((Adding.KodKv)=" + Filter.Nm + "))")
...
If Lic.fg1.TextMatrix(rw, 30) = "Да" Then

'Если счетчик то площадь равна разнице показаний счетчика
Mconn.Execute ("UPDATE Adding INNER JOIN TMP_LGOTA ON Adding.Key = TMP_LGOTA.UniKOd SET TMP_LGOTA.Plo = [Adding]![Shc_new]-[Adding]![Shc_old] WHERE (((Adding.Sch)='Да') AND ((Adding.KodKv)=" + Filter.Nm + "))")
Расчет Lic.fg1.TextMatrix(rw, 26)
End If

fg1.Row = rw

If Lic.fg1.TextMatrix(rw, 6) = "" Then Lic.fg1.TextMatrix(rw, 6) = MainForm.PrZ
Next rw

If MainForm.LGST = 1 Then Mconn.Execute ("UPDATE Adding SET Adding.LgotaP = 1 where (adding.kodkv=" + Filter.Nm + ")")
...
'FG1.SetFocus
' Скрываем колонки содержащие дни для расчета
fg1.ColHidden(45) = True
fg1.ColHidden(46) = True
...
Mconn.Execute ("DELETE Adding.*, Adding.Key From Adding WHERE ((Adding.Key)=" + fg1.TextMatrix(fg1.Row, 26) + ")")
RS.Requery
fg1.Refresh
...
Set InfoRS.ActiveConnection = Mconn
...
'Info.Show 1
Info.Caption = Lic.Caption
Info.Label1 = Lic.fg1.TextMatrix(Lic.fg1.Row, 3)
Info.Label3 = Lic.fg1.TextMatrix(Lic.fg1.Row, 2)
'Сальдо Кон Нач
Info.Label2 = Lic.fg1.TextMatrix(Lic.fg1.Row, 21)
Info.Label6 = Lic.fg1.TextMatrix(Lic.fg1.Row, 20)
'Категория расчета
Info.Label7 = Lic.fg1.TextMatrix(Lic.fg1.Row, 1)
'Тариф
Info.Label11 = Lic.fg1.TextMatrix(Lic.fg1.Row, 10)
'Соцминимум
Info.Label12 = Lic.fg1.TextMatrix(Lic.fg1.Row, 11)
'Формула
Info.Label15 = Lic.fg1.TextMatrix(Lic.fg1.Row, 19)
'общая площадь
Info.Label18 = Lic.fg1.TextMatrix(Lic.fg1.Row, 15)

'полезн площадь
Info.Label19 = Lic.fg1.TextMatrix(Lic.fg1.Row, 16)
'Прописано
Info.Label23 = Lic.fg1.TextMatrix(Lic.fg1.Row, 12)

'Проживает
Info.Label22 = Lic.fg1.TextMatrix(Lic.fg1.Row, 13)
'Key
Info.Label24 = Lic.fg1.TextMatrix(Lic.fg1.Row, 26)
...
If Lic.fg1.TextMatrix(Lic.fg1.Row, 30) = "Да" Then Расчет Lic.fg1.TextMatrix(Lic.fg1.Row, 26)
...
Me.TipArh = "opl" ' архив только оплата
'чистим arh_rep
Mconn.Execute ("DELETE arh_rep.* FROM arh_rep")
'Задаем свойства FileListBox
MainForm.File1.Path = App.Path + "\data\Arhiv\"
...
Me.TipArh = "nach" ' архив только начисления
'чистим arh_rep
Mconn.Execute ("DELETE arh_rep.* FROM arh_rep")
'Задаем свойства FileListBox
MainForm.File1.Path = App.Path + "\data\Arhiv\"
...
'//////////////////////////////////
цвет
Mconn.Execute ("UPDATE Doc SET Doc.Summa = " + Str(fg1.TextMatrix(fg1.Row, 18)) + ", Doc.Stst = 1 WHERE (((Doc.Key)=" + fg1.TextMatrix(fg1.Row, 28) + "))")
End Sub
...
Sch.Show
 ...
 If fg1.TextMatrix(fg1.Row, 49) = True Then
 Me.Command15.FontBold = True
 Me.Command15.Caption = "По нормативу"
  Me.Command15.BackColor = &H8080FF
 Else
 Me.Command15.FontBold = False
 Me.Command15.BackColor = &HFF00&
 Me.Command15.Caption = "Счетчик"
 End If
...
Sch.Label10.Caption = Round((Lic.fg1.TextMatrix(Lic.fg1.Row, 42) - Lic.fg1.TextMatrix(Lic.fg1.Row, 41)) * Lic.fg1.TextMatrix(Lic.fg1.Row, 10), 2)
Sch.Label10.Refresh
Else
...
If fg1.Row <> 0 Then
TMP_Lic.Open ("SELECT Adding.NameN, Adding.SummaI From Adding " + "Where(((Adding.KodKv) = " & Filter.Nm & ")" + " AND ((Adding.KodKat)=" + fg1.TextMatrix(fg1.Row, 22) + "))"), Mconn, adOpenForwardOnly, adLockReadOnly
...
Set Inf1.ActiveConnection = Mconn
...
Set MainOc.ActiveConnection = Mconn

MainOc.CursorType = adOpenForwardOnly

Set Perebor = New ADODB.Recordset
Set Perebor.ActiveConnection = Mconn

Set Perebor1 = New ADODB.Recordset
Set Perebor1.ActiveConnection = Mconn

Set TMP_Lic = New ADODB.Recordset
Set TMP_Lic.ActiveConnection = Mconn
...
Set Dat.ActiveConnection = Mconn
...
'Set Saldo0.ActiveConnection = Mconn
...
If Lic.a <> "Doc" Then Filter.Fg.SetFocus
Lic.a = ""
End Sub
...
Private Sub Lift_Click(Index As Integer)
...
TableWord.Cell(6, 1).Range.Text = "   Администрация " + MainForm.NamePr + " извещает, что Ваша задолженность за услуги по техническому обслуживанию лифта составляет:"



'TableWord.Cell(7, 1).Range.Text = fg1.TextMatrix(fg1.Row, 3)
...
TableWord.Cell(8, 1).Range.Text = Str(Dolg) + " (" + NumStr(Dolg, True) + ")"
'Dolg = Label10
...
Private Sub lift_izv_Click(Index As Integer)
Dim Tbl As Word.Table
Dim rng As Word.Range
Dim WordApp As Word.Application ' экземпляр приложения
Dim DocWord As Word.Document ' экземпляр документа
...
'ФИО
TableWord.Cell(2, 2).Range.Text = "гр. " + Filter.Fg.TextMatrix(Filter.Fg.Row, 2) + " " + Filter.Fg.TextMatrix(Filter.Fg.Row, 3) + " " + Filter.Fg.TextMatrix(Filter.Fg.Row, 4)
' Адрес
TableWord.Cell(3, 2).Range.Text = "г. Астрахань," + Filter.Fg.TextMatrix(Filter.Fg.Row, 5) + " Кв №" + Filter.Fg.TextMatrix(Filter.Fg.Row, 9)

'Сумма
If Val(Label10) <> 0 Then
TableWord.Cell(5, 1).Range.Text = Str(Dolg) + " руб." + "(" + NumStr(Dolg, True) + ")"
End If

' Сумма прописью

'TableWord.Cell(6, 1).Range.Text = "(" + NumStr(Dolg, True) + ")"
...
'mconn.Execute ("DELETE Adding.KodKv, Adding.KodN From Adding WHERE (((Adding.KodKv)=" + Str(Tabn) + ") AND ((Adding.KodN)=" + Str(NaKod) + "))")
Mconn.Execute ("")
MainOc.MoveNext
Loop
...
Private Sub Постоянные_начисления_Click()
Command9_Click
End Sub
...
Mconn.Execute ("UPDATE Adding INNER JOIN Nachisleniy ON Adding.KodN = Nachisleniy.Kod SET Adding.NameN = [Nachisleniy]![Naim], Adding.KodKat = [Nachisleniy]![КодKategor], Adding.Formula = [Nachisleniy]![Formula], Adding.FormulaB = [Nachisleniy]![FormulaB], Adding.Tip = [Nachisleniy]![Tip], Adding.NameKat = [Nachisleniy]![Kategor] " + ComboQ)
' Дата расчета
...
Mconn.Execute ("UPDATE Adding INNER JOIN Tarif ON (Tarif.KodDOM = Adding.TipDomKod) AND (Tarif.KodKV = Adding.TipKvKod) AND (Adding.KodKat = Tarif.KodKat) SET Adding.Tarif = [Tarif]![Value], Adding.norm = [Tarif]![norm]" + ComboQ)
'Сальдо
'Заполнить статус ИСПРАВЛЕНО 0 если небыло исправлений вручную
Mconn.Execute ("UPDATE Adding SET Adding.ispr = 0 WHERE (((Adding.ispr)<>1))")
...
'Лиготируемое да /нет
Mconn.Execute ("UPDATE Adding INNER JOIN Nachisleniy ON Adding.KodN = Nachisleniy.Kod SET Adding.Lig = [Nachisleniy]![Lig]")
RS.Requery
End Sub
...
If TMP.Fields("NLODGER").Value <> "" Then
fg1.TextMatrix(fg1.Row, 12) = TMP.Fields("NLODGER").Value
Else
fg1.TextMatrix(fg1.Row, 12) = 0 'Кол-во прописанных
End If

'If TMP.Fields("NLODGEF") = "" Then TMP.Fields("NLODGEF") = 0 'Кол-во прописанных
...
'mconn.Execute ("UPDATE Adding SET Adding.SaldoN = " + Str(Sal) + SaldoQ)
'Получить_сальдо
'Text1.Text = Sal
...
Me.Command16.Caption = "Дни" + "-" + Me.fg1.TextMatrix(fg1.Row, 45) + "/" + Me.fg1.TextMatrix(fg1.Row, 46)
...
'ЛучшаяЛгота Lic.fg1.TextMatrix(Rw, 26), True
...
For rw = 1 To fg1.Rows - 1
'MsgBox (fg1.TextMatrix(rw, 27))
'MsgBox (fg1.TextMatrix(rw, 28))
'MsgBox (fg1.TextMatrix(rw, 29))

                                  If fg1.TextMatrix(rw, 28) <> "" Then
...
' Не полные дни
If fg1.TextMatrix(rw, 45) <> fg1.TextMatrix(rw, 46) Then
fg1.Cell(flexcpBackColor, rw, 1, rw, 18) = RGB(150, 150, 150)
fg1.Cell(flexcpFontBold, rw, 18, rw, 18) = True
End If
                                                     End If
Next rw
...
'Lic.fg1.TextMatrix(Lic.fg1.Row, 46)
...
'mconn.Execute ("UPDATE Adding SET Adding.SummaI = " + FormulaN + ", Adding.ispr = 0 WHERE (((Adding.Key)=" + Str(KEY) + "))")
'mconn.Execute ("UPDATE Adding SET Adding.SummaBl = " + FormulaB + ", Adding.ispr = 0 WHERE (((Adding.Key)=" + Str(KEY) + "))")

Mconn.Execute ("UPDATE Adding SET Adding.SummaI = " + FormulaN + ", Adding.SummaBl = " + FormulaB + ", Adding.ispr = 0 WHERE (((Adding.Key)=" + Str(KEY) + "))")
...
'mconn.Execute ("UPDATE Adding SET Adding.SummaI = " + FormulaN + " WHERE (((Adding.Key)=" + Str(KEY) + ") AND ((Adding.ispr)=0))")
'mconn.Execute ("UPDATE Adding SET Adding.SummaBl = " + FormulaB + " WHERE (((Adding.Key)=" + Str(KEY) + ") AND ((Adding.ispr)=0))")

'MsgBox Oplata(1)
Mconn.Execute ("UPDATE Adding SET Adding.SummaI = " & FormulaN & ", Adding.SummaBl = " + FormulaB + " WHERE (((Adding.Key)=" + Str(KEY) + ") AND ((Adding.ispr)=0))")
             End If
...
Next I
sk = sn + Plus - Minus
Lic.Label10.Caption = Round(sn, 2)
Lic.Label11.Caption = Round(Plus, 2)
Lic.Label12.Caption = Round(Minus, 2)
Lic.Label13.Caption = Round(sk, 2)
End Sub
...
ArhivLic.Show
Set ArhivLic.VSA.DataSource = ArhivRS
Set ArhivRS = Nothing
Set ArhivCn = Nothing


For rw = 1 To ArhivLic.VSA.Rows - 1
If ArhivLic.VSA.TextMatrix(rw, 18) = "+" Then
ArhivLic.VSA.Cell(flexcpForeColor, rw, 1, rw, 18) = vbBlack
na = na + ArhivLic.VSA.TextMatrix(rw, 8)
End If
If ArhivLic.VSA.TextMatrix(rw, 18) = "-" Then
ArhivLic.VSA.Cell(flexcpForeColor, rw, 1, rw, 18) = vbRed
'ArhivLic.VSA.Cell(flexcpBackColor, Rw, 1, Rw, 18) = RGB(200, 255, 200)
Ud = Ud + ArhivLic.VSA.TextMatrix(rw, 8)
End If
If ArhivLic.VSA.TextMatrix(rw, 18) = "s" Then
ArhivLic.VSA.Cell(flexcpForeColor, rw, 1, rw, 18) = vbMagenta
Ud = Ud + ArhivLic.VSA.TextMatrix(rw, 8)
'ArhivLic.VSA.Cell(flexcpBackColor, Rw, 1, Rw, 18) = RGB(600, 255, 200)
End If
'RGB(300, 255, 200)
'RGB(200, 255, 200)
'vbGreen

sn(ArhivLic.VSA.TextMatrix(rw, 10)) = ArhivLic.VSA.TextMatrix(rw, 1)
sk(ArhivLic.VSA.TextMatrix(rw, 10)) = ArhivLic.VSA.TextMatrix(rw, 9)
Next rw

ArhivLic.Label6.Caption = Str(na)
ArhivLic.Label8.Caption = Str(Ud)
...
Next
ArhivLic.Label4.Caption = Str(SNa)
ArhivLic.Label10.Caption = Str(SKn)

'**************************
If ArhivLic.VSA.TextMatrix(1, 12) <> "" Then
ArhivLic.Label1 = ArhivLic.VSA.TextMatrix(1, 12) + " " + ArhivLic.VSA.TextMatrix(1, 13) + " " + ArhivLic.VSA.TextMatrix(1, 14) + " " + ArhivLic.VSA.TextMatrix(1, 15) + " Дом № " + ArhivLic.VSA.TextMatrix(1, 16) + " Кв №" + ArhivLic.VSA.TextMatrix(1, 17)
Else
ArhivLic.Label1 = "Нет арнивных данных"
End If
ArhivLic.Label2 = "Архивные данные за " + M + " " + Str(G) + " г."
ArhivLic.VSA.MergeCells = flexMergeRestrictAll
ArhivLic.VSA.MergeCol(-1) = True
ArhivLic.VSA.Refresh
ArhivLic.VSA.ExplorerBar = flexExMoveRows Or flexExSortShowAndMove
...
Set Fun = New ADODB.Recordset
   Fun.Open ("SELECT Sum(ADDING.SummaI) AS [Sum-SummaI] From ADDING GROUP BY ADDING.KodKat, ADDING.Tip, ADDING.KodKv HAVING (((ADDING.KodKat)=" + Str(KodKoaegorii) + ") AND ((ADDING.Tip)=" + Chr(34) + "-" + Chr(34) + "Or (ADDING.Tip)=" + Chr(34) + "s" + Chr(34) + ") AND ((ADDING.KodKv)=" + Filter.Nm + "))"), Mconn
