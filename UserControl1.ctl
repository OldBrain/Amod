...
Private StartColor As Long ' начальный цвет
Private FinishColor As Long ' конечный цвет
...
Private Function GetGradColor(lpGradient As GRADIENTSTRUCT) As Long 'получить градиентный цвет
...
Private Sub MkRGB(ByVal ColorRef As Long, Red, Green, Blue) 'извлечение RGB
...
If Highly = True Then
    DoEvents
    'здесь роасчитывается пропорции в которых нужно мешать цвета
...
Private Sub Massive() 'программа заполняет массив
Dim nX As Long, nY As Long
Dim SrcColor As Long
ReDim PixelRGB(0 To SizeX, 0 To SizeY) 'переопредиляем массив
...
Private Sub Gradient() 'рисуем из массива
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag) 'прочитаем сохраненное
