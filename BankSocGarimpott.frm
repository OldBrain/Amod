...
Begin VB.Form BankSocGarimpott 
   ...
   Begin VB.Label Label1 
      Caption         =   "Укажите путь к файлу банка. Текстовый файл по маске  *.TXT"
      Height          =   492
      Left            =   240
      TabIndex        =   3
      Top             =   120
...
stn = 0 ' Счетчик строк для отображения

'Чистим реестр ошибок
Mconn.Execute ("DELETE ErrorSocGar.* FROM ErrorSocGar")

'Признак отсутствия лиц.счета клиента в базе
NetScheta = False
...
' Cчитаем поличество строк в файле
...
Do While Not F.AtEndOfStream
 ...
 ' перебераем строки для поиска последней строки  и формируем заголовок реестра
...
Sum = TestArray(8) 'Общая сумма принятых средств
'npp = TestArray(4) 'Номер платежного поручения
DPP = TestArray(7) 'Дата зачисления
...
'Для почты в файле будут разные коды начислений поэтому пока ставим ноли
'**********************
...
Mconn.Execute ("INSERT INTO ReestrDoc ( Data, NachCod, Nach, Coment, Summa, Status, Tip, KodDom, Adres ) SELECT '" + Replace(DPP, "-", "/") + "', " + KodN + ", 'СоцГ','" + KomR + "' , 0, 0, 'Реестр СГ', 0, 'Все адреса'")
...
' (0)d_paid_coupon;(1)lsnum;(2)uk_num;(3)address;(4)fio;(5)service;(6)service_1;(7)dtz_pay;(8)summa;(10)dt_sent
    ...
    Dpl = Trim(TestArray(7)) ' Дата платежа
    Filial = " "
    'TestArray (2) 'Номер отделения
    Ls = Trim(TestArray(2)) ' Лицевой счет
        'Проверяем счет на кол-во символов и ведущие ноли
        '
            
            
     Me.Label4.Caption = "Строка > " + Str(stn) + " л/сч. " + Trim(TestArray(2)) + " ФИО " + TestArray(4) + " Адрес:" + TestArray(3)
...
' ****************************************************************
 ...
 '(10)dt_sent
    ...
    FIO = TestArray(4) 'ФИО
    Add = TestArray(3) 'Адрес
    NOtd = " " 'Номер отделения
    nkass = " " 'Номер кассира/УС/СБОЛ
    Period = Trim(TestArray(7)) ' Период оплаты
    Summ = TestArray(8) 'Сумма операции
    Summ = Replace(Summ, ",", ".")
    SummKomis = 0 ' Сумма комиссии банка
    KodN = TestArray(5) 'Код начисления соцгарантий, потом преобразуем в наш
    
    
   npp = "0"
    
 Komm = TestArray(6) + " Реестр соц.гар. " + File1.FileName
...
stn = stn + 1 ' Счетчик строк для отображения
 ...
 ' Если не найдены лиц.счета складываем в таблицу ErrorSocGar
 ...
 Me.Label4.Caption = "СЧЕТ НЕ НАЙДЕН В БАЗЕ " + Trim(TestArray(2)) + " ФИО " + TestArray(4) + " Адрес:" + TestArray(3) + " ФОРМИРУЮ РЕЕСТР ОШИБОК"
  
  
  Me.Label4.Refresh
 Mconn.Execute ("INSERT INTO ErrorSocGar ( d_paid_coupon, lsnum, uk_num, address, fio, service, service_1, dtz_pay, summa, dt_sent ) SELECT '" + TestArray(0) + "' AS Выражение1, '" + TestArray(1) + "' AS Выражение2, '" + TestArray(2) + "' AS Выражение3, '" + TestArray(3) + "' AS Выражение4, '" + TestArray(4) + "' AS Выражение5, '" + TestArray(5) + "' AS Выражение6, '" + TestArray(6) + "' AS Выражение7, '" + TestArray(7) + "' AS Выражение8, '" + TestArray(8) + "' AS Выражение9, '" + TestArray(9) + "' AS Выражение10")
...
' Сначала проставляем код начисления из файла настроек BankNastr
...
' Проставляем фамилии если в реестре соцгарантий их нет
...
MsgBox ("При разноске реестра были найдены лицевые счета несоответствующие счетам в нашей базе! Платежи этих абонентов НЕ РАЗНЕСЕНЫ !!!! Создан реестр расхождений. Необходимо распечатать реестр и доразнести платежи вручную.")
Pod.Show
Analizlgot.G = 10
sq = "SELECT ErrorSocGar.* FROM ErrorSocGar"
Reports.sq = sq
Analizlgot.Titl = "Лицевые счета не найдены в базе! Прийдеься разнести вручную."
